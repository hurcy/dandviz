...
meansc = [];
mapdata = {};
// html code block of detail 
var detail =
    '<span class="unadjusted_detail"> \
                  <p><strong><span>Observed math score</span></strong></p> \
                  <p><b><span id="ms">m</span></b></p> \
                  <p><span id="m1">m</span></p> \
                  <p><span id="m2">m</span></p> \
                  <p><span id="m3">m</span></p> \
                  <p><span id="m4">m</span></p> \
              </span> \
              <span class="adjusted_detail"> \
                  <p><strong><span>Adjusted math score</span></strong></p> \
                  <p><b><span id="msa">m</span></b></p> \
                  <p><b><span id="rk">m</span></b></p> \
              </span>'
var data = {};
// draw mercator map with popup
// apply different colors according to their membership
...
function drawLegend() {
        ...
        svg = d3.select("#legend")
            ...
            .attr("width", 600)
            .attr("height", 70);
        // map continent's name into colors
    ...
    //draw slopegraph with data in target dom
...
function drawSlopeGraph(data, target) {
        ...
        d3.select(target)
            .datum(data)
            .call(slopeGraph);
        // apply mouse event listners to labels and lines
        ...
        var projection = d3.selectAll(".slope-line")
            .on("mouseover", mouseover)
            .on("mouseout", mouseout);
        // append detail code block next to the slopegraph
        d3.select("#detail")
            .remove();
        d3.select(target)
            .append("span")
        ...
        function mouseover(d) {
            d3.select('.chart-group')
                ...
                // set active only if mouse cursor on the element
                country_labels.classed("active", function(p) {
                    return p === d;
                });
            }
            var idx = d[0];
            // ranking messages in detail box
            // apply different class if they are increased or decreased
            ...
            // fill information in detail box
    ...
    // load data from pisa.csv
d3.csv("data/pisa.csv", function(error, pisa) {
    // fill up data containers
    pisa.forEach(function(d) {
        if (d['country_w_escs'] != 'OECD average') {
            ...
            mapdata[d['ccode']]['coverage_index'] = d['coverage_index']
        }
    });
    data = {
    ...
    $("a[href='#g2']")
        .on('shown.bs.tab', function(e) {
            ...
            // apply different classes on slope lines
    ...
    $("a[href='#g4']")
        .on('shown.bs.tab', function(e) {
            ...
            // apply different classes on slope lines
            // emphasize decending countries
            d3.selectAll(".slope-line")
                .attr("class", function(d, i) {
                    slope = d[0] - d[1]
                    if (slope >= 0) return "slope-line-i-blur" + ' slope-line'
